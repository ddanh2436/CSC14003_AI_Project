...
class WorldHistoryICA(ImperialistCompetitiveAlgorithm):
    """
    World History ICA - Phiên bản 'Rise & Fall & Multipolarity' (Massive Scale).
    """
    def __init__(self, problem, pop_size=100, n_empires=10, max_year=2026, **kwargs):
        ...
        self.history_timeline = [
            ...
            (1991, "Liên Xô tan rã -> Thế giới ĐA CỰC", "MULTIPOLAR_WORLD"),
    ...
    def _apply_event_effect(self, event_type, empire_colonies, empires_fit, empires_pos):
        ...
        # --- 1991: BÙNG NỔ ĐA CỰC (SỬA ĐỔI LỚN) ---
            ...
            # 2. HỒI SINH TẤT CẢ CÁC NƯỚC NHỎ (MASS REVIVAL)
            # Lấy danh sách các vai chính
            major_powers = [usa_idx, ussr_idx, china_idx]
            
            # Gán thêm vai EU và Ấn Độ cho có cực lớn
            eu_idx = self._assign_role('EU', exclude_ids=major_powers)
            india_idx = self._assign_role('INDIA', exclude_ids=major_powers + [eu_idx])
            major_powers.extend([eu_idx, india_idx])
            
            base_power = empires_fit[usa_idx] * 5.0 # Mốc sức mạnh chuẩn

            # Vòng lặp hồi sinh: Duyệt qua TẤT CẢ đế quốc trong danh sách
            for i in range(self.n_empires):
                # Nếu không phải là siêu cường, hãy hồi sinh nó!
                if i not in major_powers:
                    # Random vị trí mới (để xuất hiện khắp nơi trên bản đồ)
                    empires_pos[i] = np.random.uniform(self.problem.bounds[:,0], self.problem.bounds[:,1])
                    # Gán sức mạnh trung bình (đủ để vẽ lên bản đồ, nhưng yếu hơn Mỹ/Trung)
                    # Random sức mạnh để nước to nước nhỏ khác nhau
                    empires_fit[i] = base_power * np.random.uniform(2.0, 10.0)
                    
                    # Nếu nó đang "chết" (fitness > 1e8), reset lại để nó "sống"
                    if empires_fit[i] > 1e8:
                        empires_fit[i] = base_power * 10.0

            # 3. Phân chia đất trống của Liên Xô cho các nước mới này tranh giành
            # (Tự động diễn ra ở các vòng lặp sau theo cơ chế ICA)
    ...
    def _evolve(self):
        ...
        for t in range(self.max_iter):
            ...
            for i in range(self.n_empires):
                # Bỏ qua nước chết
                if empires_fit[i] > 1e8 or len(empire_colonies[i]['pos']) == 0: 
                    continue
